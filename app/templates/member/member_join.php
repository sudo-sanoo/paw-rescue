...
$stmt = $conn->prepare("SELECT application_id, status, created_at FROM rescuer_applications WHERE user_id = ? ORDER BY created_at DESC LIMIT 1");
...
<div id="view-join" class="hidden animate-fade-in max-w-4xl mx-auto">
    ...
    <?php if ($has_application): ?>
        ...
        <?php
        $container_border_color = 'border-gray-100';
        $header_bg_gradient = 'from-green-100 via-gray-100 to-amber-100';
        $header_border_color = 'border-gray-200';

        if (isset($app_data['status'])) {
            if ($app_data['status'] === 'pending') {
                $container_border_color = 'border-amber-200';
                $header_bg_gradient = 'from-green-50 via-amber-100 to-amber-300';
                $header_border_color = 'border-amber-300';
            } elseif ($app_data['status'] === 'approved') {
                $container_border_color = 'border-green-200';
                $header_bg_gradient = 'from-amber-50 via-green-100 to-green-300';
                $header_border_color = 'border-green-300';
            } else {
                $container_border_color = 'border-red-200';
                $header_bg_gradient = 'from-gray-50 via-red-100 to-red-300';
                $header_border_color = 'border-red-300';
            }
        }
        ?>

        <div class="max-w-full mx-auto mt-8 bg-white rounded-2xl shadow-sm border <?= $container_border_color ?> overflow-hidden">
            
            <div class="bg-gradient-to-r <?= $header_bg_gradient ?> p-8 text-center border-b <?= $header_border_color ?>">
                
                <?php if ($app_data['status'] === 'pending'): ?>
                    <div class="w-20 h-20 bg-amber-100 text-amber-600 rounded-full flex items-center justify-center mx-auto mb-6 shadow-md">
                        <i data-lucide="hourglass" class="w-10 h-10"></i>
                    </div>
                    <h2 class="text-2xl font-bold text-gray-800 mb-2">Application Under Review</h2>
                    <p class="text-gray-600 text-sm leading-relaxed">
                        Thank you for stepping up to become a PawRescue Hero.<br>
                        Our team is currently reviewing your verification documents.
                    </p>

                <?php elseif ($app_data['status'] === 'approved'): ?>
                    ...
                    <h2 class="text-2xl font-bold text-gray-800 mb-2">You are a Hero!</h2>
                    <p class="text-gray-500 text-sm">
                        Your application has been approved. You are now an official rescuer.
                    </p>
                ...
                <?php else: ?>
                    ...
                    <p class="text-gray-600 text-sm">
                        There was an issue with your application. Please check your email for details.
                    </p>
                <?php endif; ?>
            ...
            <div class="p-8">
                <div class="bg-gray-50 rounded-xl p-6 border border-gray-100">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <?php else: ?>
                            <span class="inline-flex items-center gap-2 px-3 py-1 bg-red-50 text-red-700 rounded-full text-xs font-bold border border-red-100">
                                Rejected
                            </span>
                        <?php endif; ?>
                    </div>
